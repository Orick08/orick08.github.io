...
export function XPWindow({ title, children, className = "" }) {
  return (
    <div className={`flex flex-col bg-[#0d0d0d] rounded overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.8)] border border-[#1a1a1a] ${className}`}>
      ...
      <div className="bg-gradient-to-b from-[#ff6600] via-[#ff6600] to-[#e65c00] px-3 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-semibold text-sm drop-shadow-md">{title}</span>
        </div>
        <div className="flex gap-1.5">
          <button className="w-6 h-6 bg-white/10 hover:bg-white/20 backdrop-blur-sm transition-all flex items-center justify-center text-white text-xs font-bold rounded-sm border border-white/20">
            <span className="mb-1">_</span>
          </button>
          <button className="w-6 h-6 bg-white/10 hover:bg-white/20 backdrop-blur-sm transition-all flex items-center justify-center text-white text-xs font-bold rounded-sm border border-white/20">
            □
          </button>
          <button className="w-6 h-6 bg-white/10 hover:bg-red-500 transition-all flex items-center justify-center text-white text-xs font-bold rounded-sm border border-white/20">
            ×
          </button>
        </div>
      </div>
